...
void MainWindow::getPluginsList()
{
    ...
    //![bian li wen jian lie biao]
    foreach(QFileInfo filter, filters)
    {
        //![jian cha chajian wenjianjia zhong de mei ge wenjian ,quebao zhi jieshou chajian(ku)wenjian]
        if(QLibrary::isLibrary(filter.absoluteFilePath()))
        {
            ...
            if(dynamic_cast<CvPluginInterface*>(pluginLoader.instance()))
            {
                ui->filtersList->addItem(
                            filter.fileName());
                pluginLoader.unload();//now,unload
            }
            else
            {
                QMessageBox::warning(
...
void MainWindow::on_helpButton_pressed()
{
    if(ui->filtersList->currentRow() >= 0)
    {
        ...
        if(plugin)
        {
            ...
            QMessageBox::warning(this, tr("Warning"),
                                 QString(tr("Make sure plugin %1"
                                            "exists and is usalbe."))
                                 .arg(ui->filtersList->currentItem()->text()));
        }
    }
    else {
        QMessageBox::warning(this, tr("Warning"),
                             QString(tr("First select a filter"
